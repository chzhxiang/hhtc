  ...
  <div class="park">
    ...
    <div class="content">
      ...
      <div class="scroll" :style="{height:max_height}">
        <div v-if="isNoPark&&ajaxParkInfo.beginDay&&ajaxParkInfo.endDay&&ajaxParkInfo.beginTime&&ajaxParkInfo.endTime" class="nopark">没有搜索到您想要的车位</div>
        <div v-for="park in parkList">
          <div v-if="park.isFind&&ajaxParkInfo.beginDay&&ajaxParkInfo.endDay&&ajaxParkInfo.beginTime&&ajaxParkInfo.endTime" class="similarpark">为您匹配相近的车位</div>
          <div class="park_list">
            <div class="list_content l">
              ...
              <p class="list_day fcb">{{getParkDays(park.publishFromDates)}}</p>
      ...
      getParkList:function(){
        ...
        $.ajax({
          ...
          data:{
            ...
            publishFromTime:this.ajaxParkInfo.beginTime.replace(":",""),
            publishEndTime:this.ajaxParkInfo.endTime.replace(":","")
          },
          success:function(res){
            if(res.code==0){
...
.tab{width: 7.5rem;height:1rem;padding-top: .4rem;position:relative;}
.tab ul{width:7rem;height: .6rem;}
.tab ul li{display:block;float: left;height:.5rem;line-height:.5rem;margin:0 .18rem;width:1.2rem;text-align: center;border-bottom: 2px #0098db solid;}
.tab ul li:nth-child(1){width:1.4rem;}
.tab ul li:nth-child(2){width:1.4rem;}
.tab ul li a{display: block;line-height: .3rem;font-size: .28rem; color: #535353;}
.tab .clearn{width:.4rem;padding:0 .05rem;position:absolute;right:.2rem;top:50%;transform:translateY(-50%);-webkit-transform:translateY(-50%); background:#0098db;text-align:center;border-radius:.1rem;}
