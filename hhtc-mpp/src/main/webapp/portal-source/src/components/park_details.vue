  ...
  <div class="park_details">
    ...
    <div class="content">
      <div class="park_info fcw">
        <div class="title font38">{{parkInfo.carParkNumber|removedSemicolon}}</div>
        <div class="info"><span class="l font32">车牌号:</span><span class="r font24 add_plate" @click="goRouter('/license_plate')">{{carNumber|carShow}}</span></div>
        <div class="info"><span class="l font32">最早入场时间:</span><span class="r font24">{{parkGetBeginDay(parkInfo)}}</span></div>
        <div class="info"><span class="l font32">最迟出场时间:</span><span class="r font24">{{parkGetEndDay(parkInfo)}}</span></div>
        <div class="info"><span class="l font32">停车费:</span><span class="r font24">约 ¥ <em class="font50">{{parkInfo.price*1}}</em></span></div>
        <!--<div class="info"><span class="l font32">补押金:</span><span class="r font24">¥ <em class="font50">{{(isenough.money||0)*1}}</em></span></div>-->
      </div>
      ...
      <div class="prompt fcg95">
        ...
        <p class="font22">1、预约成功后，最早入场时间前1小时以上可将已预约车位二次发布到“抢车位”的车位预约列表中;</p>
        <p class="font22">2、请按时离场，超时停车将扣缴押金作为罚金来弥补车位主的损失和支付额外产生的管理费用;</p>
        <p class="font22">3、有任何疑问请拨打电话15023194640</p>
        <!--<p class="font22">4、</p>-->
      </div>
    ...
    <div class="ceng ceng_result fcg95 font26" style="display:none">
      <div class="ceng_content centered">
        ...
        <p class="word font26">请拨打客服电话：15023194640</p>
    ...
    <div class="ceng nomoney ceng_s fcg95 font26" style="display:none;" @click="close('nomoney')">
      <div class="ceng_content centered" @click="emptyClick($event)">
        ...
        <!--<div class="word font24" style="text-align:center;">其中押金{{fundsIsenough.moneyBase*1||0}}元停车费{{fundsIsenough.moneyRent*1||0}}元</div>-->
        <div class="ceng_operating fcw font34"><span @click="goRouter('/recharge?type=12&moneyBase='+fundsIsenough.moneyBase+'&moneyRent='+fundsIsenough.moneyRent)">去充值</span></div><!--<span @click="suer_wx">微信支付</span>-->
      </div>
    </div>
  </div>
</template>
...
module.exports={
    ...
    activated:function(){
      ...
      this.payType=2;
      this.showExclude(this.parkInfo)
      //this.getParkInfo();
      //this.ids=getUrl().ids
    },
    created:function(){
    ...
    methods:{
      ...
      getIsenoughInfo:function(carNumber){
        var that=this;
        $.ajax({
        ...
        $.ajax({
          ...
          success:function(res){
            if(res.code==0){
              ...
              //that.getIsenoughInfo(that.carNumber);
...
.park_info{width:7rem;height:5.2rem;background:url("../assets/images/park_info_bg.png") no-repeat center;background-size:7rem 5.2rem;margin:.32rem auto .2rem;}
