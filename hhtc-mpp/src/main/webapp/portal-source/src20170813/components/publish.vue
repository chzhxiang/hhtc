  ...
  <div class="my_park">
    ...
    <div class="content">
      ...
      <div class="has" v-show="parkList.length>0">
        <div class="park_list fcg95 font22" :class="park.carAuditStatus==2&&park.isUsed==2?'curr':''" v-for="park in parkList" @click="goPublish(park)"><span class="fcb font32">{{park.communityName}}</span><span>{{park.carParkNumber}}</span><span>{{getStatus(park)}}</span></div>
      </div>
      <div class="add_park font32 fcw" @click="goDistrict()">去添加车位</div>
    </div>
  </div>
</template>
...
module.exports={
    ...
    methods:{
      ...
      goDistrict:function(){
        if(this.fansInfo.carParkAuditStatus){
          if(this.fansInfo.carParkAuditStatus==2){
            this.goRouter("/district");
          }else{
            error_msg("身份审核中,请稍后再试",2000)
          }
        }else{
          error_msg("请先注册成为车位主",2000)
          this.goRouter("/login?userType=2");
        }
      }
    }
  }
...
.my_park .top{height:2.04rem;width:7rem;border-bottom-right-radius:.16rem;border-bottom-left-radius:.16rem;background:#0098db;line-height:.64rem;padding:.3rem 0 0 .7rem;position:absolute;top:0;z-index:2;}
...
.park_list span{display:inline-block;}
