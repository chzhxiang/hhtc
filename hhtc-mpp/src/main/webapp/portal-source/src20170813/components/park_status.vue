  ...
  <div class="park_status">
    <div class="bod">
      <div class="scroll">
        ...
        <div class="word"><span class="l fcg43">小区名称:</span><span class="r fcgb1 chose_district"><input type="text" name="" placeholder="请选择小区" readonly @click="choseName" v-model="nowName.name?nowName.name:showName"></span></div>
        <div class="word"><span class="l fcg43">车位号:</span><span class="r fcg43"><input class="number inputnumber" type="text" name="" placeholder="请输入您车位号" v-model="carParkNumber"></span></div>
      ...
      <div class="time_list fcg95 font26" v-if="nowStatus>=4&&parkInfoList.length>0">
        <div class="fcg43 time_list_title">历史发布时段</div>
          <div class="time_lists" v-for="park in parkInfoList">
            ...
            <div class="bottom info"><span class="l">时间: {{getParkDay(park.publishFromDate)+" 至 "+getParkDay(park.publishEndDate)}}</span></div>
            <div class="btn centered_y fcw" @click="stopPark(park)" v-if="getParkNowType(park.status)=='未锁定'">停止共享</div>
    ...
    <div class="ceng ceng_check ceng_check_w fcg95 font26" style="display:none;" @click="close('ceng_check_w')">
      <div class="ceng_content centered" @click="emptyClick($event)">
        <div class="title fcb font38">确定不共享该时段吗</div>
        <p class="word fcg43 font22"><span class="left">放租模式：</span><span class="fcb">{{getParkType(stopParkInfo.publishType)}}</span></p>
        <p class="word fcg43 font22"><span class="left">放租时间：</span><span class="fcb">{{getParkDay(stopParkInfo.publishFromDate)}} 至 {{getParkDay(stopParkInfo.publishEndDate)}}</span></p>
...
module.exports={
    ...
    activated:function(){
      ...
      this.carParkNumber=this.parkInfo.carParkNumber;
    ...
    methods:{
      ...
      getParkInfo:function(){
        ...
        if(this.scrollInfo.pd){
          ...
          $.ajax({
            url:urlDir+"/wx/goods/publish/listByGoodsId",
            type:"GET",
            data:{
      ...
      suer:function(){
        ...
        var that=this;
        $.ajax({
          url:urlDir+"/wx/goods/publish/cancel",
          type:"POST",
          data:{
...
.park_now_status{position:absolute;top:0;right:.1rem;line-height:.5rem;color:#0098db;}
</style>
