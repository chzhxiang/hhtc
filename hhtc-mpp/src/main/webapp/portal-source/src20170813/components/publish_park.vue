<template>
  <div class="publish_park">
    ...
    <div class="content">
      <div class="tab font32"><span :class="timeType==1?'curr':''" @click="changeTimeType(1)">白天</span><span :class="timeType==2?'curr':''" @click="changeTimeType(2)">过夜</span><span :class="timeType==3?'curr':''" @click="changeTimeType(3)">全天</span></div>
      <div class="time_chose fcg43 font32">
        <div><span class="l">开始时间:</span><span class="r"><input type="text" name="" id="birthday" readonly placeholder="请选择开始时间"></span></div>
        <div><span class="l">结束时间:</span><span class="r"><input type="text" name="" id="birthday1" readonly placeholder="请选择结束时间"></span></div>
        <div class="last"><span class="l">时段:</span><span class="r" id="birthday3">{{endTime}}</span><span class="r" id="birthday2">{{beginTime}}</span></div>
      </div>
    ...
    <div class="ceng ceng_check ceng_check_w fcg95 font26" style="display:none;" @click="close('ceng_check_w')">
      <div class="ceng_content centered" @click="emptyClick($event)">
        ...
        <p class="word fcg43 font22"><span class="left">放租时间：</span><span class="fcb">{{publishInfo.dayWord}}</span></p>
...
module.exports={
    ...
    mounted:function(){
      ...
      $("#birthday,#birthday1").on("click",function(){
        ...
        if($(this).attr("id")=="birthday1"){
          var minTime=$("#birthday").val();
          if(minTime){
            that.setDayList(new Date(minTime.replace(/-/g,"/")).getTime());
          }else{
        ...
        setTimeout(function(){
          $("#education_list2").mobiscroll().treelist({
            ...
            onSelect: function (valueText, inst) {
              var arr=valueText.split(" ");
              $($this).val($("#education_list2").find(".year").eq(arr[0]).text()+"-"+$("#education_list2").find(".year").eq(arr[0]).siblings("ul").find(".month").eq(arr[1]).text()+"-"+$("#education_list2").find(".year").eq(arr[0]).siblings("ul").find(".month").eq(arr[1]).siblings("ul").find(".day").eq(arr[2]).text());
              if($("#birthday").val().replace(/-/g,"")>$("#birthday1").val().replace(/-/g,"")){
                $("#birthday1").val("")
              }
    ...
    methods:{
      ...
      setDayList:function(minTime){
        ...
        $(".tab").find("span").eq(this.timeType-1).click();
      },
      ...
      goExclude:function(){
        var beginDay=$("#birthday").val(),endDay=$("#birthday1").val();
        if(!beginDay||!endDay){
          error_msg("请选择开始时间和结束时间",2000);
          return true;
        }
      ...
      suer:function(){
        ...
        $.ajax({
          ...
          data:{
            ...
            publishDates:this.effectiveWord.join(",").replace(/-/g,"").replace(/,/g,"-"),
          },
          success:function(res){
            if(res.code==0){
...
.publish_park .top{height:2.04rem;width:7rem;border-bottom-right-radius:.16rem;border-bottom-left-radius:.16rem;background:#0098db;line-height:.64rem;padding:.3rem 0 0 .7rem;position:absolute;top:0;z-index:2;}
