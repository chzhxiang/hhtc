<template>
  <div class="license_plate">
    ...
    <div class="content">
      ...
      <div class="choseplate"><span class="" @click="choseCar">{{carHead||"请选择"}}</span><span><input type="text" name="" placeholder="请输入车牌后五位" v-model="carNumberRear"></span></div>
      ...
      <div class="time_chose fcg43 font32" v-show="carNmberList.length>0">
        <div v-for="carNmber in carNmberList" @click="goCheckout(carNmber)"><span class="l">车牌</span><span class="r">{{carNmber}}</span></div>
      </div>
    </div>
...
module.exports={
    ...
    methods:{
      ...
      checkout:function(){
        var that=this;
        if(!this.carHead||!this.carNumberRear){
...
.choseplate span input{display:inline-block;width:100%;height:100%;padding-left:.44rem;background:#fff;}
