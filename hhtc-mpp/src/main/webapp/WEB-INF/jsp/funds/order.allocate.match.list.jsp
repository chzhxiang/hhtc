...
function allocate(ids){
    if(!allocating){
        allocating = true;
        $.post("${ctx}/order/allocate",
            {orderId:'${orderId}', phoneNo:'${phoneNo}', ids:ids},
            function(data){
                allocating = false;
                if(0 == data.code){
                    alert("操作成功");
                    window.location.href='${ctx}/order/matchNewGoods?phoneNo=${phoneNo}&orderId=${orderId}';
                }else{
                    $.promptBox(data.msg, "#ffb848");
                }
            }
...
<div class="c_content">
    ...
    <!--/Table list-->
    <!--Table 商品-->
    <table class="tab_head tab_list" width="100%">
        <tr class="ti"><th colspan="7"><span class="fl">匹配結果（订单编号：${orderId}）</span></th></tr>
        <tr class="nt">
            <th>ids</th>
            <th>车位号</th>
            <th>起止日期</th>
        ...
        <c:forEach items="${matchList}" var="match">
            <tr>
                <td><span>${match.ids}</span></td>
                <td><span>${match.carParkNumber}</span></td>
                <td><span>${match.dateStartEnd}</span></td>
                <td><span>${match.timeStartEnd}</span></td>
                <td><span>${match.publishType eq 1 ? "日间" : match.publishType eq 2 ? "夜间" : "全天"}</span></td>
                <c:choose>
                    <c:when test="${match.paymoney gt 0}">
                        <td><span class="cf30 fw">${match.price}</span></td>
                    </c:when>
                    <c:otherwise>
                        <td><span>${match.price}</span></td>
                    </c:otherwise>
                </c:choose>
                <td><a class="c09f mr_15" href="javascript:allocate('${match.ids}');">调配</a></td>
            </tr>
        </c:forEach>
    </table>
    <!--/Table 商品-->
